          ...
          <tbody className="divide-y divide-slate-200 font-mono text-[12px] font-bold">
            {[...accounts, ...(mt5Account ? [mt5Account] : [])].map((acc) => (
              <tr key={acc.id} className="hover:bg-slate-100/50 even:bg-slate-50/30 transition-colors group/row h-14">
                ...
                <td className={tableCellPadding + " tabular-nums"}>
                  <div className={`inline-flex items-center justify-end px-2 py-1 rounded-md border font-extrabold text-[13px] min-w-[100px] ${acc.profit >= 0 ? 'bg-success/10 border-success/20 text-success' : 'bg-danger/10 border-danger/20 text-danger'}`}>
                        ...
                        return (
                          <tr key={trade.id} className="hover:bg-slate-100/50 even:bg-slate-50/30 transition-all duration-300 group/row">
                            ...
                            <td className={tableCellPadding + " text-right"}>
                               <div className={`inline-flex items-center justify-end px-2 py-1 rounded-md border tabular-nums font-bold min-w-[100px] ${isProfitable ? 'bg-success/10 border-success/20 text-success' : 'bg-danger/10 border-danger/20 text-danger'}`}>

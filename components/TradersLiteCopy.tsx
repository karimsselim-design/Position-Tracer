...
const TradersLiteCopy: React.FC<{ theme: 'dark' | 'light' }> = ({ theme }) => {
...
const tableCellPadding = settings.layoutDensity === 'compact' ? "px-3 py-2" : settings.layoutDensity === 'relaxed' ? "px-6 py-5" : "px-4 py-3.5";
  ...
  return (
    <div className={`flex flex-col h-screen overflow-hidden transition-colors duration-300 ${settings.theme === 'dark' ? 'bg-[#05070a] text-slate-400' : 'bg-slate-50 text-slate-600'}`}>
            ...
            <div className={`${settings.layoutDensity === 'compact' ? 'px-4 py-2' : settings.layoutDensity === 'relaxed' ? 'px-10 py-8' : 'px-6 py-4'} border-b flex flex-col md:flex-row justify-between items-center gap-4 shrink-0 ${settings.theme === 'dark' ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-slate-50'}`}>
              ...
              {(activeSubTab === 'trades' || activeSubTab === 'history') && (
                <div className="relative flex-1 max-w-[240px]">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-100 border border-slate-200 rounded-lg py-1.5 pl-8 pr-3 text-[9px] font-bold outline-none focus:border-primary/40 transition-all text-slate-900"
                  />
                </div>
              )}
              ...
              <div className="flex items-center gap-4">
                ...
                <button 
                  ...
                  className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-slate-900 transition-all"
                >
                          ...
                          <tr key={trade.id} className="hover:bg-slate-100/50 even:bg-slate-50/30 transition-all duration-300 group/row">
                            ...
                            <td className={tableCellPadding + " text-center"}>
                              <button 
                                ...
                                className={`px-2 py-1 rounded-md text-[9px] font-bold tracking-wider block w-full text-center border truncate transition-colors ${trade.direction === TradeDirection.BUY ? 'bg-success/20 text-success border-success/30 hover:bg-success/30' : 'bg-danger/20 text-danger border-danger/30 hover:bg-danger/30'}`}
                              >
                                {trade.direction}
                              </button>
                            </td>
      ...
      {/* Footer Ticker */}
      <footer className={`shrink-0 h-9 border-t overflow-hidden flex items-center relative z-20 transition-colors ${settings.theme === 'dark' ? 'bg-black border-white/5' : 'bg-white border-slate-200'}`}>
        <div className={`hidden sm:flex px-6 border-r h-full items-center z-10 shrink-0 ${settings.theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
        ...
        <div className="flex-1 overflow-hidden relative h-full flex items-center">
          <div className={`animate-ticker flex items-center gap-16 text-[10px] font-bold uppercase tracking-widest pr-16 hover:[animation-play-state:paused] ${settings.theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`} style={{ animationDuration: `${localNewsSpeed}s` }}>
               {doubledNews.length > 0 ? (
                 doubledNews.map((item, idx) => (
                   ...
                   >
                     ...
                     <span className={settings.theme === 'dark' ? 'text-slate-300' : 'text-slate-800'}>{item.title}</span>
                     <span className={`${settings.theme === 'dark' ? 'text-slate-700' : 'text-slate-300'} font-black opacity-20 px-2`}>//</span>
                   </div>
                 ))
               ) : (
        ...
        <div className={`hidden sm:flex px-4 items-center gap-2 border-l z-10 h-full shrink-0 ${settings.theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-slate-200'}`}>
          <button onClick={() => {
          ...
          <span className={`text-[9px] font-black w-4 text-center ${settings.theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>{localNewsSpeed}</span>
      ...
      {(isAddingTrade || editingTrade) && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-6 bg-slate-900/40 dark:bg-black/80 backdrop-blur-xl animate-in fade-in duration-300">
          <div className={`w-full max-w-[700px] glass-panel rounded-[2.5rem] border shadow-2xl ${settings.theme === 'dark' ? 'border-white/10 bg-[#05070a]' : 'border-slate-200 bg-white'}`}>
            <header className={`px-10 py-8 border-b flex justify-between items-center rounded-t-[2.5rem] ${settings.theme === 'dark' ? 'border-white/5 bg-white/5' : 'border-slate-100 bg-slate-50'}`}>
              <div>
                <h2 className={`text-xl font-black uppercase tracking-tighter ${settings.theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
                  {editingTrade ? 'Modify Deployment' : 'Cluster Order Deployment'}
                </h2>
              </div>
              <button onClick={() => { setIsAddingTrade(false); setEditingTrade(null); }} className={`size-8 flex items-center justify-center rounded-full transition-all border ${settings.theme === 'dark' ? 'bg-white/5 text-slate-500 hover:text-white border-white/5' : 'bg-slate-200 text-slate-500 hover:text-slate-900 border-slate-300'}`}>
            ...
            <form onSubmit={handleAddTradeSubmit} className="p-8 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <select 
                    ...
                    className={`w-full border rounded-xl py-2 px-4 text-sm font-mono focus:border-primary outline-none ${settings.theme === 'dark' ? 'bg-black/60 border-white/10 text-white' : 'bg-slate-100 border-slate-200 text-slate-900'}`}
                  >
                    {MARKET_ASSETS.map(asset => <option key={asset.id} value={asset.id} className={settings.theme === 'dark' ? 'bg-slate-900' : 'bg-white'}>{asset.name}</option>)}
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Bias Direction</label>
                  <div className={`flex p-1 rounded-xl border ${settings.theme === 'dark' ? 'bg-black/60 border-white/10' : 'bg-slate-100 border-slate-200'}`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input type="number" step="any" value={newTrade.entry} onChange={e => setNewTrade({...newTrade, entry: Number(e.target.value)})} className={`w-full border rounded-xl py-2 px-4 text-sm font-mono focus:border-primary outline-none ${settings.theme === 'dark' ? 'bg-black/60 border-white/10 text-white' : 'bg-slate-100 border-slate-200 text-slate-900'}`} />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Lot Size</label>
                  <input type="number" step="0.01" value={newTrade.lotSize} onChange={e => setNewTrade({...newTrade, lotSize: Number(e.target.value)})} className={`w-full border rounded-xl py-2 px-4 text-sm font-mono focus:border-primary outline-none ${settings.theme === 'dark' ? 'bg-black/60 border-white/10 text-white' : 'bg-slate-100 border-slate-200 text-slate-900'}`} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input type="number" step="any" value={newTrade.sl} onChange={e => setNewTrade({...newTrade, sl: Number(e.target.value)})} className={`w-full border rounded-xl py-2 px-4 text-sm font-mono focus:border-primary outline-none ${settings.theme === 'dark' ? 'bg-black/60 border-white/10 text-white' : 'bg-slate-100 border-slate-200 text-slate-900'}`} />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Take Profit</label>
                  <input type="number" step="any" value={newTrade.tp1} onChange={e => setNewTrade({...newTrade, tp1: Number(e.target.value)})} className={`w-full border rounded-xl py-2 px-4 text-sm font-mono focus:border-primary outline-none ${settings.theme === 'dark' ? 'bg-black/60 border-white/10 text-white' : 'bg-slate-100 border-slate-200 text-slate-900'}`} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input type="number" value={newTrade.dayTarget} onChange={e => setNewTrade({...newTrade, dayTarget: Number(e.target.value)})} className={`w-full border rounded-xl py-2 px-4 text-sm font-mono focus:border-primary outline-none ${settings.theme === 'dark' ? 'bg-black/60 border-white/10 text-white' : 'bg-slate-100 border-slate-200 text-slate-900'}`} />
                </div>
                <div className="space-y-1">
                  ...
                  {!isAddingNewTrader ? (
                    ...
                    <div className="flex gap-2">
                      ...
                      <button 
                        ...
                        className={`px-3 rounded-xl text-xs font-bold transition-all ${settings.theme === 'dark' ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-slate-200 text-slate-600 hover:bg-slate-300'}`}
                      >
                        Cancel
                      </button>
                    </div>
              ...
              {!editingTrade && (
                <div className={`flex items-center gap-3 p-4 border rounded-xl ${settings.theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-slate-100 border-slate-200'}`}>
                  <input 
                    type="checkbox" 
                    id="executeOnMT5" 
                    checked={executeOnMT5} 
                    onChange={(e) => setExecuteOnMT5(e.target.checked)}
                    className={`size-4 rounded accent-primary cursor-pointer ${settings.theme === 'dark' ? 'border-white/10 bg-black/40' : 'border-slate-300 bg-white'}`} 
                  />
                  <label htmlFor="executeOnMT5" className={`text-[10px] font-black uppercase tracking-widest cursor-pointer flex-1 ${settings.theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>
...
export default TradersLiteCopy;
